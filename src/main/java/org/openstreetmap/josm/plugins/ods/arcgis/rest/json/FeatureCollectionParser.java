...
public class FeatureCollectionParser {
    ...
    public SimpleFeatureCollection parse(InputStream is)
        ...
        try (
            ...
            // (SimpleFeatureType)odsFeatureSource.getFeatureType();
    ...
    private SimpleFeature parseFeature(JsonParser jp)
            throws JsonParseException, IOException {
        JsonNode node = mapper.readTree(jp);
    ...
    private static Object getValue(JsonNode node, Class<?> binding) {
        if (node.isNull()) {
            return null;
        }
        if (node.isTextual()) {
    ...
    protected Point parsePoint(JsonNode node) {
        ...
        coordinate.x = node.get("x").doubleValue();
        coordinate.y = node.get("y").doubleValue();
        return geoFactory.createPoint(coordinate);
    }

    protected Point parseMultiPoint(JsonNode node) {
        JsonNode points = node.get("points");
        Point[] thePoints = new Point[points.size()];
        for (int i = 0; i < points.size(); i++) {
            JsonNode point = points.get(i);
            Coordinate coordinate = new Coordinate();
            coordinate.x = point.get(0).doubleValue();
            coordinate.y = point.get(1).doubleValue();
            thePoints[i] = geoFactory.createPoint(coordinate);
        }
        return thePoints[0];
    }
